...
public class frmLogin : MetroForm
{
	...
	public frmLogin()
	{
		...
		{
			IME = (string.IsNullOrEmpty(GetIME().Trim()) ? "null#Rs232" : (GetIME() + "#Rs232"))
		};
	}
	...
	private void RefreshFromSocket(string json)
	{
		...
		Invoke((MethodInvoker)delegate
		{
			if (item.Id == mId)
			{
				...
				{
					frmResultView frmResultView2 = new frmResultView(this, item);
					frmResultView2.Show();
				}
			}
			string text = "yyyy/MM";
	...
	private void LoadLiences()
	{
		new Task(delegate
		{
			...
			try
			{
				ResponseDto result = client.GetsAsync(client.IME.Replace("#", ""), "/api/Machine/GetLiences/{id}").Result;
	...
	private void main_resultToolStripMenuItem_Click(object sender, EventArgs e)
	{
		...
		new frmResultView(this, request).Show();
	}
