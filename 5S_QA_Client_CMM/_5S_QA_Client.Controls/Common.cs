...
public static class Common
{
	...
	{
		string text = Path.Combine(".\\", "Mappers");
	...
	public static bool addMappers(Form frm, Guid productid, DataGridView dgvmain, DataGridView dgvcontent)
	{
		...
		try
		{
			...
			if (dgvcontent != null)
			{
				...
				{
					MessageBox.Show(getTextLanguage(frm, "HasExist") + ": " + mappername, getTextLanguage(frm, "WARNING"), MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
					return false;
				}
			...
			else
			{
				foreach (DataGridViewRow row in (IEnumerable)dgvmain.Rows)
				{
					...
					{
						MessageBox.Show(string.Format("{0}: {1} {2} {3}", getTextLanguage(frm, "HasExist"), mappername2, getTextLanguage(frm, "AtRow"), row.Cells["no"].Value), getTextLanguage(frm, "WARNING"), MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
						return false;
					}
				}
			}
		...
		{
			MessageBox.Show(ex.Message, getTextLanguage(frm, "ERROR"), MessageBoxButtons.OK, MessageBoxIcon.Hand);
			return false;
		}
	}
	...
	public static List<LanguageDto> ReadLanguages(string path)
	{
