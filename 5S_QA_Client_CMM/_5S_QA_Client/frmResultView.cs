...
public class frmResultView : MetroForm
{
	...
	private readonly string mDirResult = ConfigurationManager.ConnectionStrings["DirResult"].ConnectionString;
	...
	private const int ROW_MAPPER = 2;

	private const int ROW_VALUE = 3;

	private const int ROW_UNIT = 4;

	private const int ROW_UPPER = 5;

	private const int ROW_LOWER = 6;

	private const int ROW_TOOL = 7;

	private const int ROW_SAMPLE_FIRST = 8;
	...
	{
		dgvMain.Rows[8].Cells[0].Selected = true;
	...
	private void Init()
	{
		...
		Load_Settings();
		Text = Text + " (" + frmLogin.mMachineName + "#" + frmLogin.mMachineType + " * " + mRequest.Code + " * " + mRequest.Name + ")";
	...
	private void Init_dgvMain(DataTable dt)
	{
		...
		List<RowDto> list = new List<RowDto>
		{
			...
			{
				Code = "Mapper",
				Display = Common.getTextLanguage(this, "Mapper"),
				ReadOnly = true,
				IsCenter = true
			},
			new RowDto
	...
	private void Init_Button()
	{
		new Task(delegate
		{
			Invoke((MethodInvoker)delegate
			{
				...
				Directory.CreateDirectory(mDirResult);
				switch (frmLogin.mMachineType)
				{
				case "CLP-35":
					fileWatcher.Filter = "*.LN2";
					break;
				case "ADCOLE911":
	...
	private void Init_ImageProduct()
	{
		new Task(delegate
		{
			...
			mPanelImage.Init(ptbProduct.Image, mRequest.Id);
		}).Start();
	}
	...
	private CellDto Get_Cell(dynamic result)
	{
		...
		for (int i = 1; i < dgvMain.ColumnCount; i++)
		{
			...
			{
				if (dgvMain.Rows[2].Cells[i].Value.ToString() == result.Name)
				{
					num = i;
					break;
				}
			}
			else if (dgvMain.Rows[0].Cells[i].Value.ToString() == result.MeasurementId.ToString())
			{
				num = i;
				break;
			}
		}
		for (int j = 8; j < dgvMain.RowCount; j++)
		{
	...
	private void Cal_Judge()
	{
		int num = 0;
		int num2 = 0;
		for (int i = 8; i <= ROW_SAMPLE_LAST; i++)
		{
	...
	private bool Save_Result(string machineName = "Manual Input")
	{
		...
		if (double.TryParse(dgvMain.Rows[3].Cells[dgvMain.CurrentCellAddress.X].Value.ToString(), out var result2) && !string.IsNullOrEmpty(text))
		{
			...
			double.TryParse(dgvMain.Rows[5].Cells[dgvMain.CurrentCellAddress.X].Value.ToString(), out var result4);
			double.TryParse(dgvMain.Rows[6].Cells[dgvMain.CurrentCellAddress.X].Value.ToString(), out var result5);
		...
		else if (dgvMain.Rows[4].Cells[dgvMain.CurrentCellAddress.X].Value?.ToString() == "°")
		{
			text = Common.ConvertDegreesToDouble(text)?.ToString();
		}
		try
		{
			Guid id = ((AuditableEntity)(object)Get_Result())?.Id ?? Guid.Empty;
			ResultViewModel val = new ResultViewModel();
			((AuditableEntity)val).Id = id;
			val.RequestId = mRequest.Id;
			val.MeasurementId = Guid.Parse(dgvMain.Rows[0].Cells[dgvMain.CurrentCellAddress.X].Value.ToString());
			val.Result = text;
			val.MachineName = machineName;
			val.StaffName = frmLogin.User.FullName;
			val.Sample = int.Parse(lblSample.Text);
			val.Cavity = int.Parse(lblCavity.Text);
			((AuditableEntity)val).IsActivated = true;
			ResultViewModel body = val;
			Cursor.Current = Cursors.WaitCursor;
	...
	private bool Get_CellIsValue(int row, int col, out int rowout, out int colout, bool isVertical = false)
	{
		bool result = false;
		if (isVertical)
		{
	...
	private void Next_Result(bool isVertical = false)
	{
		int row = dgvMain.CurrentCellAddress.Y;
		int col = dgvMain.CurrentCellAddress.X;
		while (Get_CellIsValue(row, col, out row, out col, isVertical))
		{
		}
	...
	private int Check_ColSameTool(int column)
	{
		...
		{
			string text = dgvMain.Rows[7].Cells[i].Value.ToString();
				...
				if (list.Count >= 1)
				{
					...
					for (col = 1; col < dgvMain.ColumnCount; col++)
					{
						...
						if (aql != null)
						{
							...
							for (int num2 = 8; num2 <= ROW_SAMPLE_LAST; num2++)
							{
	...
	private void drawMerge(DataTable dt)
	{
		new Task(delegate
		{
			Invoke((MethodInvoker)delegate
			{
				...
				int num = 0;
				int columnIndex = 0;
				List<MapperMeasDto> mappers = Common.getMappers(mRequest.ProductId);
				int i;
				for (i = 0; i < dt.Rows.Count; i++)
				{
					MapperMeasDto val = mappers.Where((MapperMeasDto x) => x.MeasurementId.Equals(Guid.Parse(dt.Rows[i]["Id"].ToString()))).FirstOrDefault();
					if (val != null && !string.IsNullOrEmpty(val.Mapper))
					{
						dgvMain[i + 1, 2].Value = val.Mapper;
						dgvMain[i + 1, 2].Style.ForeColor = Color.White;
						dgvMain[i + 1, 2].Style.BackColor = Color.Gray;
					}
					...
					if (text2.Contains("LENGTH") || text2.Contains("WIDTH") || text2.Contains("THICKNESS"))
					{
						...
						else
						{
							...
							for (int num3 = ROW_SAMPLE_LAST + 1; num3 < dgvMain.RowCount; num3++)
							{
								((DataGridViewTextBoxCellEx)dgvMain[columnIndex, num3]).ColumnSpan = num;
								dgvMain[columnIndex, num3].Style.Alignment = DataGridViewContentAlignment.MiddleCenter;
							}
							num = 1;
							text = text2;
							columnIndex = i + 1;
						...
						if (i == dt.Rows.Count - 1 && num > 1)
						{
							...
							{
								((DataGridViewTextBoxCellEx)dgvMain[columnIndex, num4]).ColumnSpan = num;
								dgvMain[columnIndex, num4].Style.Alignment = DataGridViewContentAlignment.MiddleCenter;
							}
							for (int num5 = ROW_SAMPLE_LAST + 1; num5 < dgvMain.RowCount; num5++)
							{
								((DataGridViewTextBoxCellEx)dgvMain[columnIndex, num5]).ColumnSpan = num;
								dgvMain[columnIndex, num5].Style.Alignment = DataGridViewContentAlignment.MiddleCenter;
							}
						}
					}
					else if (num > 1)
					{
						...
						{
							((DataGridViewTextBoxCellEx)dgvMain[columnIndex, num6]).ColumnSpan = num;
							dgvMain[columnIndex, num6].Style.Alignment = DataGridViewContentAlignment.MiddleCenter;
						}
						for (int num7 = ROW_SAMPLE_LAST + 1; num7 < dgvMain.RowCount; num7++)
						{
							((DataGridViewTextBoxCellEx)dgvMain[columnIndex, num7]).ColumnSpan = num;
							dgvMain[columnIndex, num7].Style.Alignment = DataGridViewContentAlignment.MiddleCenter;
						}
						num = 0;
						text = text2;
						columnIndex = i + 1;
					...
					{
						((DataGridViewTextBoxCellEx)dgvMain[i + 1, 3]).RowSpan = 4;
						dgvMain[i + 1, 3].Style.Alignment = DataGridViewContentAlignment.MiddleCenter;
						dgvMain[i + 1, 3].Style.WrapMode = DataGridViewTriState.True;
					}
				}
			});
		}).Start();
	...
	{
		if (main_importToolStripMenuItem.Text != Common.getTextLanguage(this, "Import"))
		{
			for (int i = 1; i < dgvMain.ColumnCount; i++)
			{
				for (int j = 8; j <= ROW_SAMPLE_LAST; j++)
				{
					if (!(dgvMain.Rows[j].Cells[i].Style.ForeColor != Color.Green))
					{
						dgvMain.Rows[j].Cells[i].Value = null;
					}
				}
			}
		}
		...
		btnImport.Text = main_importToolStripMenuItem.Text;
	}

	private void setResultsToDGV()
	{
		try
		{
			if (Results == null || Results.Count == 0)
			{
				throw new Exception(Common.getTextLanguage(this, "Unsuitable"));
			}
			mProgressBarMain.Value = 0;
			mProgressBarMain.Maximum = Results.Count;
			bool flag = false;
			foreach (Result result in Results)
			{
				mProgressBarMain.Value++;
				CellDto cellDto = Get_Cell(result);
				if (cellDto != null && !dgvMain[cellDto.Column, cellDto.Row].ReadOnly)
				{
					dgvMain[cellDto.Column, cellDto.Row].Value = result.Actual;
					dgvMain[cellDto.Column, cellDto.Row].Style.ForeColor = Color.Green;
					flag = true;
				}
			}
			if (flag)
	...
	private bool saveResults()
	{
		...
		Invoke((MethodInvoker)delegate
		{
			Cursor.Current = Cursors.WaitCursor;
			sprogbarStatus.Value = 0;
			debugOutput(Common.getTextLanguage(this, "Importing"));
		});
		List<ResultViewModel> list = new List<ResultViewModel>();
		foreach (Result result3 in Results)
		{
			CellDto cellDto = Get_Cell(result3);
			if (cellDto != null && !dgvMain[cellDto.Column, cellDto.Row].ReadOnly)
			{
				Guid id = ((AuditableEntity)(object)Get_Result())?.Id ?? Guid.Empty;
				ResultViewModel val = new ResultViewModel
				{
					Result = result3.Actual.ToString()
				};
				((AuditableEntity)val).Id = id;
				val.RequestId = mRequest.Id;
				val.MeasurementId = Guid.Parse(dgvMain[cellDto.Column, 0].Value.ToString());
				((AuditableEntity)val).IsActivated = true;
				val.MachineName = frmLogin.mMachineName;
				val.StaffName = lblFullname.Text;
				val.Sample = result3.Sample;
				val.Cavity = result3.Cavity;
				list.Add(val);
			}
		}
		ResponseDto result2 = frmLogin.client.SaveAsync(list, "/api/RequestResult/SaveList").Result;
	...
	private void timerMain_Tick(object sender, EventArgs e)
	{
		...
		try
		{
			int.TryParse(lblSample.Text, out var result);
			int.TryParse(lblCavity.Text, out var result2);
			if (fileNames.Count > 0 && result != 0 && result2 != 0)
			{
				Results = null;
				switch (frmLogin.mMachineType)
				{
				case "CLP-35":
					Results = MachineFilter.FilterResult_CLP_35(fileNames.First(), result, result2);
					break;
				case "ADCOLE911":
					Results = MachineFilter.FilterResult_ADCOLE911(fileNames.First(), result, result2);
					break;
				case "ROUNDNESS-ACCTee":
					Results = MachineFilter.FilterResult_ROUNDNESS_ACCTee(fileNames, result, result2);
					break;
				case "CONTOUR-ACCTee":
					Results = MachineFilter.FilterResult_CONTOUR_ACCTee(fileNames, result, result2);
					break;
				case "SUFRCOM-ACCTee":
					Results = MachineFilter.FilterResult_SUFRCOM_ACCTee(fileNames, result, result2);
					break;
				case "CRYSTA":
					Results = MachineFilter.FilterResult_CRYSTA(fileNames.First(), result, result2);
					break;
				case "SE3500":
					Results = MachineFilter.FilterResult_SE3500(fileNames.First(), result, result2);
					break;
				case "EF3000":
					Results = MachineFilter.FilterResult_EF3000(fileNames.First(), result, result2);
					break;
				}
				if (saveResults())
				{
					Next_Result(isVertical: true);
				}
			}
			else
			{
				debugOutput(Common.getTextLanguage(this, "Unsample"));
			}
		}
		finally
		{
	...
	private void OnChanged(object sender, FileSystemEventArgs e)
	{
		...
		fileWatcher.WaitForChanged(WatcherChangeTypes.Changed, 300);
		if (File.Exists(e.FullPath) && dgvMain.CurrentCell != null)
		{
			if (!fileNames.Contains(e.FullPath))
			{
	...
	private void dgvMain_CurrentCellChanged(object sender, EventArgs e)
	{
		...
		if (dgvMain.CurrentCell != null)
		{
			if (dgvMain.CurrentCellAddress.X != 0)
			{
				...
				if (cbShowChart.Checked && mChartNumber > 0 && dgvMain.Rows[3].Cells[dgvMain.CurrentCellAddress.X].Value != dgvMain.Rows[4].Cells[dgvMain.CurrentCellAddress.X].Value)
				{
			...
			lblTool.Text = dgvMain.Rows[7].Cells[dgvMain.CurrentCellAddress.X].Value.ToString();
	...
	private void dgvMain_EditingControlShowing(object sender, DataGridViewEditingControlShowingEventArgs e)
	{
		...
		dataGridViewTextBoxEditingControl.KeyPress += txtNormal_KeyPress;
		if (dgvMain.Rows[3].Cells[dgvMain.CurrentCellAddress.X].Value == dgvMain.Rows[4].Cells[dgvMain.CurrentCellAddress.X].Value)
		{
	...
	private void dgvMain_CellEndEdit(object sender, DataGridViewCellEventArgs e)
	{
		...
		if (array.Length > 1)
		{
			...
			MachineLienceViewModel val = mForm.mMachines.FirstOrDefault((MachineLienceViewModel x) => x.Mark == int.Parse(mark));
			if (val == null)
			{
				dgvMain.CurrentCell.Value = mRemember;
			...
			{
				string text = dgvMain.Rows[7].Cells[dgvMain.CurrentCellAddress.X].Value.ToString();
				if (val.MachineType == text)
				{
					dgvMain.CurrentCell.Value = array.First();
					Save_Result(val.Name);
				}
				else
				{
					dgvMain.CurrentCell.Value = mRemember;
		...
		if (dgvMain.Rows[3].Cells[e.ColumnIndex].Value != dgvMain.Rows[4].Cells[e.ColumnIndex].Value)
		{
			Next_Result();
		}
	}
	...
	{
		if (dgvMain.CurrentCell != null && dgvMain.Rows[3].Cells[dgvMain.CurrentCellAddress.X].Value == dgvMain.Rows[4].Cells[dgvMain.CurrentCellAddress.X].Value)
		{
	...
	{
		Settings.Default.IsAuto = cbAuto.Checked;
		Settings.Default.Save();
		fileWatcher.Changed -= OnChanged;
		if (!string.IsNullOrEmpty(mDirResult) && cbAuto.Checked)
		{
			fileWatcher.Changed += OnChanged;
		}
	}
	...
	private void main_importToolStripMenuItem_Click(object sender, EventArgs e)
	{
		...
		if (main_importToolStripMenuItem.Text.Equals(Common.getTextLanguage(this, "Import")))
		{
			...
			{
				MessageBox.Show(Common.getTextLanguage(this, "Unsample"), Common.getTextLanguage(this, "WARNING"), MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
				panelmProgressBarMain.Visible = false;
				return;
			}
			switch (frmLogin.mMachineType)
			{
			case "CLP-35":
				openFileDialogMain.Filter = "File result (*.LN2)|*.LN2";
				break;
			case "ADCOLE911":
				openFileDialogMain.Filter = "File result (*.txt)|*.txt";
				break;
			case "ROUNDNESS-ACCTee":
			...
			case "SE3500":
				...
				break;
			case "EF3000":
				openFileDialogMain.Filter = "File result (*.csv)|*.csv";
				break;
			default:
				MessageBox.Show(Common.getTextLanguage(this, "Unname"), Common.getTextLanguage(this, "WARNING"), MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
				panelmProgressBarMain.Visible = false;
				return;
			}
			if (openFileDialogMain.ShowDialog().Equals(DialogResult.OK))
			{
				...
				{
				case "CLP-35":
					Results = MachineFilter.FilterResult_CLP_35(openFileDialogMain.FileName, result, result2);
					break;
				case "ADCOLE911":
					Results = MachineFilter.FilterResult_ADCOLE911(openFileDialogMain.FileName, result, result2);
					break;
				case "ROUNDNESS-ACCTee":
					Results = MachineFilter.FilterResult_ROUNDNESS_ACCTee(openFileDialogMain.FileNames.ToList(), result, result2);
					break;
				case "CONTOUR-ACCTee":
					Results = MachineFilter.FilterResult_CONTOUR_ACCTee(openFileDialogMain.FileNames.ToList(), result, result2);
					break;
				case "SUFRCOM-ACCTee":
					Results = MachineFilter.FilterResult_SUFRCOM_ACCTee(openFileDialogMain.FileNames.ToList(), result, result2);
					break;
				case "CRYSTA":
					Results = MachineFilter.FilterResult_CRYSTA(openFileDialogMain.FileName, result, result2);
					break;
				case "SE3500":
					Results = MachineFilter.FilterResult_SE3500(openFileDialogMain.FileName, result, result2);
					break;
				case "EF3000":
					Results = MachineFilter.FilterResult_EF3000(openFileDialogMain.FileName, result, result2);
					break;
				}
				setResultsToDGV();
			}
	...
	private void InitializeComponent()
	{
		...
		this.btnComplete.Location = new System.Drawing.Point(1063, 1);
		this.btnComplete.Margin = new System.Windows.Forms.Padding(0);
		this.btnComplete.Name = "btnComplete";
		this.btnComplete.Size = new System.Drawing.Size(96, 30);
		...
		this.cbSameTool.Location = new System.Drawing.Point(577, 4);
		this.cbSameTool.Name = "cbSameTool";
		this.cbSameTool.Size = new System.Drawing.Size(87, 24);
		this.cbSameTool.TabIndex = 6;
		...
		this.btnEnterOK.Location = new System.Drawing.Point(668, 1);
		...
		this.btnEnterNG.Location = new System.Drawing.Point(730, 1);
		...
		this.btnRefresh.Size = new System.Drawing.Size(702, 30);
		...
		this.cbAuto.Location = new System.Drawing.Point(911, 4);
		this.cbAuto.Name = "cbAuto";
		this.cbAuto.Size = new System.Drawing.Size(93, 24);
		this.cbAuto.TabIndex = 9;
		this.cbAuto.Text = "Auto import";
		this.toolTipMain.SetToolTip(this.cbAuto, "Select auto import result from folder");
		this.cbAuto.UseVisualStyleBackColor = true;
		this.cbAuto.CheckedChanged += new System.EventHandler(cbAuto_CheckedChanged);
		this.btnImport.AutoSize = true;
		this.btnImport.Cursor = System.Windows.Forms.Cursors.Hand;
		this.btnImport.Dock = System.Windows.Forms.DockStyle.Fill;
		this.btnImport.Location = new System.Drawing.Point(1008, 1);
		this.btnImport.Margin = new System.Windows.Forms.Padding(0);
		this.btnImport.Name = "btnImport";
		this.btnImport.Size = new System.Drawing.Size(54, 30);
		this.btnImport.TabIndex = 10;
		this.btnImport.Text = "Import";
		this.toolTipMain.SetToolTip(this.btnImport, "Select import or confirm result");
		this.btnImport.UseVisualStyleBackColor = true;
		this.btnImport.Click += new System.EventHandler(main_importToolStripMenuItem_Click);
		...
		this.contextMenuStripMain.Items.AddRange(new System.Windows.Forms.ToolStripItem[5] { this.main_refreshToolStripMenuItem, this.toolStripSeparator1, this.main_viewToolStripMenuItem, this.toolStripSeparator2, this.main_importToolStripMenuItem });
		...
		this.contextMenuStripMain.Size = new System.Drawing.Size(114, 82);
		...
		this.tpanelTitle.ColumnCount = 12;
		...
		this.tpanelButton.ColumnStyles.Add(new System.Windows.Forms.ColumnStyle());
		this.tpanelButton.ColumnStyles.Add(new System.Windows.Forms.ColumnStyle());
		this.tpanelButton.Controls.Add(this.btnImport, 10, 0);
		this.tpanelButton.Controls.Add(this.cbAuto, 9, 0);
		this.tpanelButton.Controls.Add(this.cbShowChart, 0, 0);
		this.tpanelButton.Controls.Add(this.btnEnterNG, 7, 0);
		this.tpanelButton.Controls.Add(this.btnEnterOK, 6, 0);
		this.tpanelButton.Controls.Add(this.cbSameTool, 5, 0);
		this.tpanelButton.Controls.Add(this.btnComplete, 11, 0);
		this.tpanelButton.Controls.Add(this.rbHorizontal, 4, 0);
		this.tpanelButton.Controls.Add(this.btnImage, 2, 0);
		this.tpanelButton.Controls.Add(this.btnHistory, 1, 0);
		this.tpanelButton.Controls.Add(this.rbVertical, 3, 0);
		...
		this.openFileDialogMain.Filter = "File csv (*.csv)|*.csv|File excel (*.xls, *.xlsx)| *.xls; *.xlsx";
		this.openFileDialogMain.Title = "Select result file";
