...
public class ViewPortHandler {
    ...
    public void limitTransAndScale(Matrix matrix, RectF content) {
        ...
        float curTransX = fArr[2];
        float curScaleX = fArr[0];
        float curTransY = fArr[5];
        float curScaleY = fArr[4];
        ...
        float max = Math.max(Math.min(curTransY, this.mTransOffsetY + ((this.mScaleY - 1.0f) * height)), -this.mTransOffsetY);
        this.mTransY = max;
