...
public class apiGetResultAsync extends AsyncTask<String, String, List<ResultViewModel>> {
    ...
    public apiGetResultAsync(Context context2, UUID id, int sample) {
    ...
    public List<ResultViewModel> doInBackground(String... strings) {
        try {
            Response response = this.okHttpClient.newCall(new Request.Builder().url(strings[0] + this.url).addHeader("Accept", "text/plain").addHeader("Authorization", "Bearer " + MainActivity.TOKEN).post(RequestBody.create(MediaType.parse("application/json"), this.json.serialize(new String[0]))).build()).execute();
