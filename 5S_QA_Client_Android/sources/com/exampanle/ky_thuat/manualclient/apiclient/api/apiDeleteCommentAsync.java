...
public class apiDeleteCommentAsync extends AsyncTask<String, String, Boolean> {
    ...
    public apiDeleteCommentAsync(Context context2, UUID id) {
        this.context = context2;
        this.url = "/api/Comment/Delete/" + id;
    }

    /* access modifiers changed from: protected */
    public Boolean doInBackground(String... strings) {
        try {
            Response response = this.okHttpClient.newCall(new Request.Builder().url(strings[0] + this.url).addHeader("Accept", "text/plain").addHeader("Authorization", "Bearer " + MainActivity.TOKEN).delete().build()).execute();
