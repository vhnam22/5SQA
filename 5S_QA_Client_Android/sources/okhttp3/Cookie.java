...
public final class Cookie {
    ...
    static Cookie parse(long currentTimeMillis, HttpUrl url, String setCookie) {
        ...
        String path4 = "/";
        if (path3 == null || !path3.startsWith(path4)) {
            String encodedPath = url.encodedPath();
            int lastSlash = encodedPath.lastIndexOf(47);
            if (lastSlash != 0) {
                path4 = encodedPath.substring(0, lastSlash);
            }
            path2 = path4;
        } else {
            path2 = path3;
            ...
            if (hour == -1 && matcher.usePattern(TIME_PATTERN).matches()) {
                ...
                if (month == -1) {
                    ...
                    if (matcher.usePattern(pattern).matches()) {
                        month = pattern.pattern().indexOf(matcher.group(1).toLowerCase(Locale.US)) / 4;
                    }
                }
    ...
    public int hashCode() {
        ...
        return (((((((((((((((((17 * 31) + this.name.hashCode()) * 31) + this.value.hashCode()) * 31) + this.domain.hashCode()) * 31) + this.path.hashCode()) * 31) + ((int) (j ^ (j >>> 32)))) * 31) + (this.secure ^ true ? 1 : 0)) * 31) + (this.httpOnly ^ true ? 1 : 0)) * 31) + (this.persistent ^ true ? 1 : 0)) * 31) + (this.hostOnly ^ true ? 1 : 0);
    }
}
