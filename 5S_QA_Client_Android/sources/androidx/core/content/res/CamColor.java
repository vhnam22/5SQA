...
class CamColor {
    ...
    public int viewed(ViewingConditions viewingConditions) {
        float alpha;
        if (((double) getChroma()) == Utils.DOUBLE_EPSILON || ((double) getJ()) == Utils.DOUBLE_EPSILON) {
            alpha = 0.0f;
        } else {
        ...
        float f = alpha;
        float f2 = t;
        float rCBase = (float) Math.max(Utils.DOUBLE_EPSILON, (((double) Math.abs(rA)) * 27.13d) / (400.0d - ((double) Math.abs(rA))));
        float f3 = p1;
        float f4 = p2;
        float rC = Math.signum(rA) * (100.0f / viewingConditions.getFl()) * ((float) Math.pow((double) rCBase, 2.380952380952381d));
        float gCBase = (float) Math.max(Utils.DOUBLE_EPSILON, (((double) Math.abs(gA)) * 27.13d) / (400.0d - ((double) Math.abs(gA))));
        float f5 = ac;
        float f6 = eHue;
        float gC = Math.signum(gA) * (100.0f / viewingConditions.getFl()) * ((float) Math.pow((double) gCBase, 2.380952380952381d));
        float bCBase = (float) Math.max(Utils.DOUBLE_EPSILON, (((double) Math.abs(bA)) * 27.13d) / (400.0d - ((double) Math.abs(bA))));
        float f7 = hRad;
        float f8 = bCBase;
    ...
    static int toColor(float hue, float chroma, float lstar, ViewingConditions viewingConditions) {
        if (((double) chroma) < 1.0d || ((double) Math.round(lstar)) <= Utils.DOUBLE_EPSILON || ((double) Math.round(lstar)) >= 100.0d) {
