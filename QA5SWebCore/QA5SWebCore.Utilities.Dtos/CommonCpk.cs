namespace QA5SWebCore.Utilities.Dtos;

public class CommonCpk
{
	public string ItemNo { get; set; }

	public string Tool { get; set; }

	public double Standard { get; set; }

	public double Upper { get; set; }

	public double Lower { get; set; }

	public int n { get; set; }

	public int nTerm { get; set; }

	public int OutUSL { get; set; }

	public int OK { get; set; }

	public int OutLSL { get; set; }

	public double Average { get; set; }

	public double Summation { get; set; }

	public double Maximun { get; set; }

	public double Minimun { get; set; }

	public double Range { get; set; }

	public double SD { get; set; }

	public double Variance { get; set; }

	public double Varition { get; set; }

	public double Cp { get; set; }

	public double HCpk { get; set; }

	public double LCpk { get; set; }

	public double Cpk { get; set; }

	public double XUCL { get; set; }

	public double XLCL { get; set; }

	public double RUCL { get; set; }

	public double RLCL { get; set; }

	public double Rtb { get; set; }

	public double USL { get; set; }

	public double UCL { get; set; }

	public double Sigma6 { get; set; }

	public double Sigma5 { get; set; }

	public double Sigma4 { get; set; }

	public double Sigma3 { get; set; }

	public double Sigma2 { get; set; }

	public double Sigma1 { get; set; }

	public double CL { get; set; }

	public double MSigma1 { get; set; }

	public double MSigma2 { get; set; }

	public double MSigma3 { get; set; }

	public double MSigma4 { get; set; }

	public double MSigma5 { get; set; }

	public double MSigma6 { get; set; }

	public double LCL { get; set; }

	public double LSL { get; set; }

	public double R { get; set; }

	public double AveR { get; set; }

	public double K { get; set; }

	public double u1 { get; set; }

	public double u2 { get; set; }

	public double Defective { get; set; }

	public double p1 { get; set; }

	public double p2 { get; set; }

	public double LHTerm { get; set; }

	public double RHTerm { get; set; }

	public double AveHTerm { get; set; }

	public double LowEnd { get; set; }

	public double HighEnd { get; set; }

	public double Min { get; set; }

	public double Max { get; set; }

	public double Diff { get; set; }

	public double Class { get; set; }
}
