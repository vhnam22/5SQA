...
public class mHistogram : UserControl
{
	...
	public mHistogram(CommonCpk commonCpk, List<OtherDataCpk> dataCpks, List<ChartViewModel> models)
	{
		InitializeComponent();
	...
	private void mHistogram_Load(object sender, EventArgs e)
	{
		Init();
	}
	...
	private void Init()
	{
		...
		chartMain.ChartAreas[0].AxisX.Minimum = num2;
		chartMain.ChartAreas[0].AxisX.Maximum = num3;
		chartMain.ChartAreas[0].AxisX.Interval = Math.Round((num3 - num2) / 10.0, 4, MidpointRounding.AwayFromZero);
		num3 = (double)mDataCpks.Max((OtherDataCpk x) => x.Frequency) * 1.02;
		chartMain.ChartAreas[0].AxisY.Minimum = 0.0;
		chartMain.ChartAreas[0].AxisY.Maximum = num3;
		double num4 = Math.Round(num3 / 10.0, 0, MidpointRounding.AwayFromZero);
		chartMain.ChartAreas[0].AxisY.Interval = ((num4 == 0.0) ? 1.0 : num4);
		foreach (OtherDataCpk mDataCpk in mDataCpks)
		{
			double xValue = Math.Round(mDataCpk.Avegare, 4, MidpointRounding.AwayFromZero);
			chartMain.Series["Frequency"].Points.AddXY(xValue, mDataCpk.Frequency);
			double num5 = mDataCpks.Max((OtherDataCpk c) => c.NormalDist);
			double yValue = 0.0;
			if (!num5.Equals(0.0))
			{
				yValue = mDataCpk.NormalDist * (double)mDataCpks.Max((OtherDataCpk c) => c.Frequency) / mDataCpks.Max((OtherDataCpk c) => c.NormalDist);
			}
			chartMain.Series["FrequencyL"].Points.AddXY(xValue, yValue);
		}
		chartMain.Series["USL"].Points.AddXY(mCommonCpk.USL, 0.0);
		chartMain.Series["LSL"].Points.AddXY(mCommonCpk.LSL, 0.0);
		chartMain.Series["UCL"].Points.AddXY(mCommonCpk.UCL, 0.0);
		chartMain.Series["LCL"].Points.AddXY(mCommonCpk.LCL, 0.0);
		chartMain.Series["Avegare"].Points.AddXY(mCommonCpk.Average, 0.0);
		chartMain.Series["USL"].Points.AddXY(mCommonCpk.USL, num3);
		chartMain.Series["LSL"].Points.AddXY(mCommonCpk.LSL, num3);
		chartMain.Series["UCL"].Points.AddXY(mCommonCpk.UCL, num3);
		chartMain.Series["LCL"].Points.AddXY(mCommonCpk.LCL, num3);
		chartMain.Series["Avegare"].Points.AddXY(mCommonCpk.Average, num3);
		if (mCommonCpk.Unit == "°")
		{
			chartMain.ChartAreas[0].AxisX.CustomLabels.Clear();
			for (double num6 = chartMain.ChartAreas[0].AxisX.Minimum; num6 <= chartMain.ChartAreas[0].AxisX.Maximum; num6 += chartMain.ChartAreas[0].AxisX.Interval)
			{
				chartMain.ChartAreas[0].AxisX.CustomLabels.Add(num6 - chartMain.ChartAreas[0].AxisX.Interval / 2.0, num6 + chartMain.ChartAreas[0].AxisX.Interval / 2.0, Common.ConvertDoubleToDegrees(num6));
			}
		}
		DrawInformation();
	...
	{
		int num = (int)((float)chartMain.Width * chartMain.ChartAreas[0].InnerPlotPosition.X / 100f);
		int num2 = (int)((float)chartMain.Width * (chartMain.ChartAreas[0].InnerPlotPosition.X + chartMain.ChartAreas[0].InnerPlotPosition.Width) / 100f);
		List<double> list = new List<double> { mCommonCpk.USL, mCommonCpk.UCL, mCommonCpk.Average, mCommonCpk.LCL, mCommonCpk.LSL }.OrderBy((double x) => x).ToList();
		...
		transparentLabel.Location = new Point(GetLocationX(num + transparentLabel.Width / 2, num2 - transparentLabel.Width / 2, 5, list.IndexOf(mCommonCpk.USL)) - transparentLabel.Width / 2, 3);
		...
		transparentLabel2.Location = new Point(GetLocationX(num + transparentLabel2.Width / 2, num2 - transparentLabel2.Width / 2, 5, list.IndexOf(mCommonCpk.LSL)) - transparentLabel2.Width / 2, 3);
		...
		transparentLabel3.Location = new Point(GetLocationX(num + transparentLabel3.Width / 2, num2 - transparentLabel3.Width / 2, 5, list.IndexOf(mCommonCpk.Average)) - transparentLabel3.Width / 2, 3);
		...
		transparentLabel4.Location = new Point(GetLocationX(num + transparentLabel4.Width / 2, num2 - transparentLabel4.Width / 2, 5, list.IndexOf(mCommonCpk.UCL)) - transparentLabel4.Width / 2, 3);
		...
		transparentLabel5.Location = new Point(GetLocationX(num + transparentLabel5.Width / 2, num2 - transparentLabel5.Width / 2, 5, list.IndexOf(mCommonCpk.LCL)) - transparentLabel5.Width / 2, 3);
	}

	private int GetLocationX(int xmin, int xmax, int number, int pos)
	{
	...
	private void InitializeComponent()
	{
		...
		this.lblR.Location = new System.Drawing.Point(787, 50);
		this.lblR.Margin = new System.Windows.Forms.Padding(3);
		this.lblR.Name = "lblR";
		this.lblR.Size = new System.Drawing.Size(64, 16);
		...
		this.label18.Location = new System.Drawing.Point(729, 50);
		...
		this.panelChart.Size = new System.Drawing.Size(1000, 273);
		this.panelChart.TabIndex = 11;
		chartArea.AlignWithChartArea = "ChartArea1";
		...
		series.Font = new System.Drawing.Font("Microsoft Sans Serif", 8f, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, 0);
		...
		this.chartMain.Size = new System.Drawing.Size(998, 271);
		...
		this.panelInfor.Size = new System.Drawing.Size(998, 0);
		...
		this.tpanelDifference.TabIndex = 12;
		...
		base.Name = "mHistogram";
		base.Size = new System.Drawing.Size(1000, 390);
		base.Load += new System.EventHandler(mHistogram_Load);
