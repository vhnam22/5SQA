...
public class mXtbSControl : UserControl
{
	...
	public mXtbSControl(CommonCpk commonCpk, List<DataCpk> dataCpks, List<ChartViewModel> models)
	{
		InitializeComponent();
	...
	private void mXtbSControl_Load(object sender, EventArgs e)
	{
		Init();
	}
	...
	private void Init()
	{
		...
		source = new List<double> { mCommonCpk.R, mCommonCpk.SD };
		...
		foreach (DataCpk mDataCpk in mDataCpks)
		{
			...
			chartMain.Series["S\u00af"].Points.AddXY(mDataCpk.Date, mCommonCpk.SD);
			chartMain.Series["SD"].Points.AddXY(mDataCpk.Date, mDataCpk.SD);
		}
		if (mCommonCpk.Unit == "°")
		{
	...
	private void DrawInformation()
	{
		...
		list = new List<double> { mCommonCpk.R, mCommonCpk.SD }.OrderByDescending((double x) => x).ToList();
		...
		transparentLabel6.Location = new Point(0, GetLocationY(num3 + transparentLabel6.Height / 2, num4 - transparentLabel6.Height / 2, 2, list.IndexOf(mCommonCpk.R)) - transparentLabel6.Height / 2);
		TransparentLabel transparentLabel7 = new TransparentLabel
		{
			Text = "S\u00af " + ((mCommonCpk.Unit == "°") ? Common.ConvertDoubleToDegrees(mCommonCpk.SD) : mCommonCpk.SD.ToString("0.0000")),
			Font = new Font("Microsoft Sans Serif", 9.75f, FontStyle.Regular, GraphicsUnit.Point, 0),
			Margin = new Padding(0),
			AutoSize = true
		};
		panelInfor.Controls.Add(transparentLabel7);
		transparentLabel7.Location = new Point(0, GetLocationY(num3 + transparentLabel7.Height / 2, num4 - transparentLabel7.Height / 2, 2, list.IndexOf(mCommonCpk.SD)) - transparentLabel7.Height / 2);
	}
	...
	private void chartMain_MouseMove(object sender, MouseEventArgs e)
	{
		if (mPreHit != null)
		{
			...
			if (mPreHit.Series.Name.Equals("Data") || mPreHit.Series.Name.Equals("SD"))
			{
		...
		if (mPreHit != null)
		{
			...
			if (mPreHit.Series.Name.Equals("Data") || mPreHit.Series.Name.Equals("SD") || mPreHit.Series.Name.Equals("ErrorBars"))
			{
	...
	private void InitializeComponent()
	{
		...
		base.Name = "mXtbSControl";
		base.Size = new System.Drawing.Size(1000, 430);
		base.Load += new System.EventHandler(mXtbSControl_Load);
