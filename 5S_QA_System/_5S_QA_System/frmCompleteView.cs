...
public class frmCompleteView : MetroForm
{
	...
	private List<RequestResultViewModel> mResults;
	...
	public frmCompleteView(RequestViewModel request)
	{
		InitializeComponent();
	...
	private void loadDecimals()
	{
		try
		{
			...
			mDecimals = Common.getObjectToList<string>(result);
		}
		catch (Exception ex)
		{
	...
	private List<RequestResultViewModel> Get_Results()
	{
		List<RequestResultViewModel> result = new List<RequestResultViewModel>();
		try
		{
			QueryArgs queryArgs = new QueryArgs();
			...
			result = Common.getObjectToList<RequestResultViewModel>(result2);
		}
		catch (Exception ex)
		{
			if (ex.InnerException is ApiException { StatusCode: var statusCode } ex2)
	...
	private RequestResultViewModel Get_Result()
	{
		...
		return mResults.FirstOrDefault((RequestResultViewModel x) => x.Sample == sample && x.Cavity == cavity && x.MeasurementId == id);
	}
	...
	private List<dynamic> Cal_dgvMain(List<RequestResultViewModel> results)
	{
		List<object> list = new List<object>();
		results = results.Where((RequestResultViewModel x) => !string.IsNullOrEmpty(x.Result)).ToList();
		List<RequestResultViewModel> source = results.Select((RequestResultViewModel x) => x.Clone()).ToList();
		...
		IEnumerable<IGrouping<Guid?, RequestResultViewModel>> enumerable = from x in source
			group x by x.MeasurementId;
		foreach (IGrouping<Guid?, RequestResultViewModel> item in enumerable)
		{
			string result = "-";
			string result2 = "-";
			string result3 = "-";
			...
			{
				List<double> list2 = item.Select((RequestResultViewModel x) => double.Parse(x.Result)).ToList();
			...
			foreach (RequestResultViewModel item2 in item)
			{
	...
	private void Set_dgvMain()
	{
		new Task(delegate
		{
			Invoke((MethodInvoker)delegate
			{
				...
				foreach (RequestResultViewModel mResult in mResults)
				{
	...
	private List<LimitViewModel> loadSampleWithAQL()
	{
		...
		try
		{
			...
			result = Common.getObjectToList<LimitViewModel>(result2);
		}
		catch (Exception ex)
		{
			if (ex.InnerException is ApiException { StatusCode: var statusCode } ex2)
	...
	private List<MeasurementQuickViewModel> getMeasurementIsRank()
	{
		...
		try
		{
			...
			QueryArgs body = queryArgs;
			ResponseDto result2 = frmLogin.client.GetsAsync(body, "/api/Measurement/Gets").Result;
			result = Common.getObjectToList<MeasurementQuickViewModel>(result2);
		}
		catch (Exception ex)
		{
			if (ex.InnerException is ApiException { StatusCode: var statusCode } ex2)
	...
	private List<ResultRankViewModel> getResultRank()
	{
		...
		try
		{
			...
			result = Common.getObjectToList<ResultRankViewModel>(result2);
		}
		catch (Exception ex)
		{
			if (ex.InnerException is ApiException { StatusCode: var statusCode } ex2)
	...
	{
		RequestResultViewModel result = Get_Result();
		if (result == null)
		{
			mPanelViewMain.Visible = false;
			return;
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(_5S_QA_System.frmCompleteView));
		...
		this.btnRefresh.Location = new System.Drawing.Point(121, 1);
		this.btnRefresh.Margin = new System.Windows.Forms.Padding(0);
		this.btnRefresh.Name = "btnRefresh";
		this.btnRefresh.Size = new System.Drawing.Size(542, 30);
		this.btnRefresh.TabIndex = 3;
		this.btnRefresh.Text = "Refresh";
		this.toolTipMain.SetToolTip(this.btnRefresh, "Refresh");
		this.btnRefresh.UseVisualStyleBackColor = true;
		this.btnRefresh.Click += new System.EventHandler(main_refreshToolStripMenuItem_Click);
		...
		this.tpanelInfor.Controls.Add(this.btnRefresh, 2, 0);
		...
		this.lblWarningDisplay.TabIndex = 19;
		...
		this.lblEditDisplay.TabIndex = 18;
		...
		this.ptbAvatar.ErrorImage = _5S_QA_System.Properties.Resources._5S_QA_S;
		...
		this.ptbAvatar.InitialImage = _5S_QA_System.Properties.Resources._5S_QA_S;
		...
		this.btnShowHeader.Image = _5S_QA_System.Properties.Resources.app;
		...
		this.Text = "5S QA System * VIEW ALL";
