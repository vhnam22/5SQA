...
public class frmTemplateOtherView : MetroForm
{
	...
	public frmTemplateOtherView(Form frm, Guid id = default(Guid))
	{
		InitializeComponent();
	...
	private void frmTemplateOtherView_Load(object sender, EventArgs e)
	{
		Init();
	}

	private void frmTemplateOtherView_Shown(object sender, EventArgs e)
	{
		load_AllData();
		mPanelViewMain.load_cbbTemplate(mIdParent);
	}

	private void frmTemplateOtherView_FormClosing(object sender, FormClosingEventArgs e)
	{
		GC.Collect();
	}

	private void frmTemplateOtherView_FormClosed(object sender, FormClosedEventArgs e)
	{
		if (!isClose)
		{
	...
	public void load_AllData()
	{
		...
		try
		{
			...
			dgvMain.DataSource = Common.getDataTable<TemplateOtherViewModel>(result);
			dgvMain.Refresh();
	...
	private void main_deleteToolStripMenuItem_Click(object sender, EventArgs e)
	{
		...
		{
			if (!MessageBox.Show(string.Format("{0} {1}", Common.getTextLanguage(this, "wSureDelete"), dgvMain.CurrentRow.Cells["TemplateName"].Value), Common.getTextLanguage(this, "QUESTION"), MessageBoxButtons.YesNo, MessageBoxIcon.Question).Equals(DialogResult.Yes))
			{
				return;
			}
			try
			{
				Cursor.Current = Cursors.WaitCursor;
				start_Proccessor();
				ResponseDto result = frmLogin.client.DeleteAsync(mId, "/api/TemplateOther/Delete/{id}").Result;
				if (!result.Success)
				{
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(_5S_QA_System.frmTemplateOtherView));
		...
		this.contextMenuStripMain.Size = new System.Drawing.Size(181, 126);
		...
		this.dgvMain.Columns.AddRange(this.No, this.ProductId, this.TemplateId, this.TemplateName, this.TemplateDescription, this.Id, this.Created, this.Modified, this.CreatedBy, this.ModifiedBy, this.IsActivated);
		...
		this.TemplateName.FillWeight = 40f;
		...
		base.Name = "frmTemplateOtherView";
		base.Padding = new System.Windows.Forms.Padding(20, 70, 20, 20);
		this.Text = "5S QA System * TEMPLATE OTHER";
		base.FormClosing += new System.Windows.Forms.FormClosingEventHandler(frmTemplateOtherView_FormClosing);
		base.FormClosed += new System.Windows.Forms.FormClosedEventHandler(frmTemplateOtherView_FormClosed);
		base.Load += new System.EventHandler(frmTemplateOtherView_Load);
		base.Shown += new System.EventHandler(frmTemplateOtherView_Shown);
