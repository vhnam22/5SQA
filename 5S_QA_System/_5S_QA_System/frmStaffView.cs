...
public class frmStaffView : MetroForm
{
	private Guid mId;
	...
	private ToolStripSeparator toolStripSeparatorLogout;

	private ToolStripMenuItem main_logoutToolStripMenuItem;
	...
	private DataGridViewTextBoxColumn IsEmail;
	...
	private DataGridViewTextBoxColumn LoginAt;

	private DataGridViewTextBoxColumn TimeLogin;
	...
	private void frmStaffView_FormClosing(object sender, FormClosingEventArgs e)
	{
		GC.Collect();
	}

	private void frmStaffView_FormClosed(object sender, FormClosedEventArgs e)
	{
		mPanelViewMain.mDispose();
		List<Type> list = new List<Type>();
		list.Add(typeof(frmStaffAdd));
	...
	public void load_AllData()
	{
		...
		try
		{
			...
			queryArgs.Predicate = "Username!=@0";
			queryArgs.PredicateParameters = new string[1] { "Admin" };
			queryArgs.Order = "Created DESC";
			queryArgs.Page = 1;
			queryArgs.Limit = int.MaxValue;
			QueryArgs body = queryArgs;
			ResponseDto result = frmLogin.client.GetsAsync(body, "/api/AuthUser/Gets").Result;
			mSearchMain.Init(Common.getDataTable<AuthUserViewModel>(result), dgvMain);
			load_dgvMain();
		}
		catch (Exception ex)
		{
	...
	private void dgvMain_CurrentCellChanged(object sender, EventArgs e)
	{
		...
		if (dataGridView.CurrentCell != null)
		{
			...
			{
				mId = guid;
				if (mId.Equals(frmLogin.User.Id) || string.IsNullOrEmpty(dataGridView.CurrentRow.Cells["LoginAt"].Value.ToString()))
				{
					toolStripSeparatorLogout.Visible = false;
					main_logoutToolStripMenuItem.Visible = false;
				}
				else
				{
					toolStripSeparatorLogout.Visible = true;
					main_logoutToolStripMenuItem.Visible = true;
				}
	...
	private void dgvMain_Sorted(object sender, EventArgs e)
	{
		...
		foreach (DataGridViewRow item in (IEnumerable)dataGridView.Rows)
		{
			...
			if (!(bool)item.Cells["IsActivated"].Value)
			{
	...
	private void main_newToolStripMenuItem_Click(object sender, EventArgs e)
	{
		...
		list.Add(typeof(frmStaffAdd));
		Common.closeForm(list);
		new frmStaffAdd(this, (DataTable)dgvMain.DataSource, mId).Show();
	}
	...
	private void main_editToolStripMenuItem_Click(object sender, EventArgs e)
	{
		...
		{
			if (dgvMain.CurrentRow.Cells["role"].Value == null || ((RoleWeb)dgvMain.CurrentRow.Cells["role"].Value/*cast due to .constrained prefix*/).Equals(RoleWeb.SuperAdministrator))
			{
				MessageBox.Show(Common.getTextLanguage(this, "wPositionLower"), Common.getTextLanguage(this, "WARNING"), MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
				return;
			}
			if (((RoleWeb)dgvMain.CurrentRow.Cells["role"].Value/*cast due to .constrained prefix*/).Equals(RoleWeb.Administrator) && !Guid.Parse(dgvMain.CurrentRow.Cells["id"].Value.ToString()).Equals(frmLogin.User.Id))
			{
				MessageBox.Show(Common.getTextLanguage(this, "wYourInformation"), Common.getTextLanguage(this, "WARNING"), MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
				return;
			}
		}
		List<Type> list = new List<Type>();
		list.Add(typeof(frmStaffAdd));
		Common.closeForm(list);
		new frmStaffAdd(this, (DataTable)dgvMain.DataSource, mId, isadd: false).Show();
	}
	...
	private void main_deleteToolStripMenuItem_Click(object sender, EventArgs e)
	{
		...
		{
			if (!MessageBox.Show(string.Format("{0} {1}", Common.getTextLanguage(this, "wSureDelete"), dgvMain.CurrentRow.Cells["FullName"].Value), Common.getTextLanguage(this, "QUESTION"), MessageBoxButtons.YesNo, MessageBoxIcon.Question).Equals(DialogResult.Yes))
			{
				return;
			}
			try
			{
				Cursor.Current = Cursors.WaitCursor;
				start_Proccessor();
				ResponseDto result = frmLogin.client.DeleteAsync(mId, "/api/AuthUser/Delete/{id}").Result;
				if (!result.Success)
				{
	...
	private void main_logoutToolStripMenuItem_Click(object sender, EventArgs e)
	{
		mSearchMain.btnSearch.Select();
		Cursor.Current = Cursors.WaitCursor;
		if (mId.Equals(Guid.Empty))
		...
		else if (!frmLogin.User.Role.Equals(RoleWeb.SuperAdministrator))
		{
			MessageBox.Show(Common.getTextLanguage(this, "wPositionLowerLogout"), Common.getTextLanguage(this, "WARNING"), MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
		}
		else
		{
			if (!MessageBox.Show(string.Format("{0} {1}", Common.getTextLanguage(this, "wSureLogout"), dgvMain.CurrentRow.Cells["FullName"].Value), Common.getTextLanguage(this, "QUESTION"), MessageBoxButtons.YesNo, MessageBoxIcon.Question).Equals(DialogResult.Yes))
			{
				return;
			}
			try
			{
				Cursor.Current = Cursors.WaitCursor;
				start_Proccessor();
				ResponseDto result = frmLogin.client.LogoutAsync(mId).Result;
				if (!result.Success)
				{
					throw new Exception(result.Messages.ElementAt(0).Message);
				}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(_5S_QA_System.frmStaffView));
		...
		this.toolStripSeparatorLogout = new System.Windows.Forms.ToolStripSeparator();
		this.main_logoutToolStripMenuItem = new System.Windows.Forms.ToolStripMenuItem();
		...
		this.IsEmail = new System.Windows.Forms.DataGridViewTextBoxColumn();
		...
		this.LoginAt = new System.Windows.Forms.DataGridViewTextBoxColumn();
		...
		this.statusStripMain.Location = new System.Drawing.Point(20, 554);
		...
		this.statusStripMain.Size = new System.Drawing.Size(1060, 26);
		...
		this.toolStripSeparatorLogout.Name = "toolStripSeparatorLogout";
		this.toolStripSeparatorLogout.Size = new System.Drawing.Size(110, 6);
		this.main_logoutToolStripMenuItem.Name = "main_logoutToolStripMenuItem";
		this.main_logoutToolStripMenuItem.Size = new System.Drawing.Size(113, 22);
		this.main_logoutToolStripMenuItem.Text = "Logout";
		this.main_logoutToolStripMenuItem.Click += new System.EventHandler(main_logoutToolStripMenuItem_Click);
		...
		this.dgvMain.Columns.AddRange(this.No, this.Username, this.FullName, this.Email, this.IsEmail, this.BirthDate, this.Gender, this.PhoneNumber, this.Address, this.JobTitle, this.Position, this.DepartmentId, this.DepartmentName, this.ImageUrl, this.LoginAt, this.TimeLogin, this.Role, this.Token, this.Id, this.Created, this.Modified, this.CreatedBy, this.ModifiedBy, this.IsActivated);
		...
		this.IsEmail.DataPropertyName = "IsEmail";
		this.IsEmail.HeaderText = "Receive email";
		this.IsEmail.Name = "IsEmail";
		this.IsEmail.ReadOnly = true;
		this.IsEmail.Visible = false;
		...
		this.Position.FillWeight = 25f;
		this.Position.HeaderText = "Permission";
		...
		this.DepartmentName.FillWeight = 25f;
		...
		this.LoginAt.AutoSizeMode = System.Windows.Forms.DataGridViewAutoSizeColumnMode.Fill;
		this.LoginAt.DataPropertyName = "LoginAt";
		this.LoginAt.FillWeight = 20f;
		this.LoginAt.HeaderText = "Login at";
		this.LoginAt.Name = "LoginAt";
		this.LoginAt.ReadOnly = true;
		...
		this.TimeLogin.FillWeight = 25f;
		this.TimeLogin.HeaderText = "Time";
		...
		base.Name = "frmStaffView";
		base.Padding = new System.Windows.Forms.Padding(20, 70, 20, 20);
		this.Text = "5S QA System * STAFF";
		base.FormClosing += new System.Windows.Forms.FormClosingEventHandler(frmStaffView_FormClosing);
		base.FormClosed += new System.Windows.Forms.FormClosedEventHandler(frmStaffView_FormClosed);
		base.Load += new System.EventHandler(frmStaffView_Load);
		base.Shown += new System.EventHandler(frmStaffView_Shown);
