...
public static class Common
{
	...
	public static bool addMappers(Guid productid, DataGridView dgvmain, DataGridView dgvcontent)
	{
		try
		{
			...
			if (dgvcontent != null)
			{
				...
				MapperMeasDto mapperMeasDto = list.Where((MapperMeasDto x) => x.Mapper.Equals(mappername) && !x.MeasurementId.Equals(Guid.Parse(dgvmain.CurrentRow.Cells["Id"].Value.ToString()))).FirstOrDefault();
				if (mapperMeasDto != null && !string.IsNullOrEmpty(mappername))
				{
					MessageBox.Show("Mapper name: [" + mappername + "] already exit.", "WARNING", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
					return false;
				}
				mapperMeasDto = list.Where((MapperMeasDto x) => x.MeasurementId.Equals(Guid.Parse(dgvmain.CurrentRow.Cells["Id"].Value.ToString()))).FirstOrDefault();
				mapperMeasDto.Mapper = mappername;
			}
			else
			{
				foreach (DataGridViewRow row in (IEnumerable)dgvmain.Rows)
				{
					...
					MapperMeasDto mapperMeasDto2 = list.Where((MapperMeasDto x) => x.Mapper.Equals(mappername2) && !x.MeasurementId.Equals(Guid.Parse(row.Cells["Id"].Value.ToString()))).FirstOrDefault();
					if (mapperMeasDto2 != null && !string.IsNullOrEmpty(mappername2))
					{
	...
	public static void setControls(Control control, ToolTip tooltip = null, List<ContextMenuStrip> contexts = null)
	{
		...
		LanguageDto languageDto = list.FirstOrDefault((LanguageDto x) => control.Name.Equals(x.Code));
		if (languageDto != null)
		{
			object value = languageDto.GetType().GetProperty(name).GetValue(languageDto, null);
			if (value != null)
			{
		...
		{
			languageDto = list.FirstOrDefault((LanguageDto x) => ctrl.Name.Replace("lbl", "txt").Equals(x.Code));
			if (languageDto != null)
			{
				object value2 = languageDto.GetType().GetProperty(name).GetValue(languageDto, null);
				if (value2 != null)
				{
				...
				{
					value2 = languageDto.GetType().GetProperty(name2).GetValue(languageDto, null);
					if (value2 != null)
					{
			...
			{
				languageDto = list.FirstOrDefault((LanguageDto x) => col.Name.Equals(x.Code));
				if (languageDto != null)
				{
					object value3 = languageDto.GetType().GetProperty(name).GetValue(languageDto, null);
					if (value3 != null)
					{
		...
		foreach (ContextMenuStrip context in contexts)
		{
			foreach (object item in context.Items)
			{
				...
				languageDto = list.FirstOrDefault((LanguageDto x) => _menu.Name.StartsWith(x.Code));
				if (languageDto != null)
				{
					object value4 = languageDto.GetType().GetProperty(name).GetValue(languageDto, null);
					if (value4 != null)
					{
	...
	public static string getTextLanguage(Control control, string code)
	{
		...
		{
			string name = Settings.Default.Language.Replace("rb", "Name");
			LanguageDto languageDto = list.FirstOrDefault((LanguageDto x) => code.Equals(x.Code));
			if (languageDto != null)
			{
				object value = languageDto.GetType().GetProperty(name).GetValue(languageDto, null);
				if (value != null)
