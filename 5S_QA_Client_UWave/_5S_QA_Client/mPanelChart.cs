...
public class mPanelChart : UserControl
{
	...
	public void Set_Chart(_5S_QA_Entities.Dtos.ResultForChartDto dto, int limit)
	{
		mChartNumber = limit;
	...
	public void Init(Guid id)
	{
		...
		foreach (IGrouping<Guid?, ChartViewModel> item in enumerable)
		{
			...
			dataCpk.UpperErrorBars = dataCpk.Max - dataCpk.Average;
			dataCpk.LowerErrorBars = dataCpk.Average - dataCpk.Min;
			datacpkAlls.Add(dataCpk);
		}
	...
	private List<ChartViewModel> Load_Data(_5S_QA_Entities.Dtos.ResultForChartDto dto)
	{
	...
	private double GetConstant(int n, string title)
	{
		...
		try
		{
			_5S_QA_Entities.Dtos.QueryArgs queryArgs = new _5S_QA_Entities.Dtos.QueryArgs();
			queryArgs.Predicate = "n=@0";
			queryArgs.PredicateParameters = new string[1] { n.ToString() };
			queryArgs.Order = "Created";
			queryArgs.Page = 1;
			queryArgs.Limit = 1;
			_5S_QA_Entities.Dtos.QueryArgs body = queryArgs;
